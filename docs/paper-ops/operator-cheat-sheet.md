...
2. **Start**
3. **Enable strategy**
4. **Tick** (repeat) — watch orders / fills / lots / cash / P&L
5. Optional: **Pause** / **Resume** / **Stop**
...
- `strategy_mode`
- `registry.model_id` / `registry.state` (Model Qualification)
...
**Qualification (local demo):** bind a registry model in process before
`advisory` / `iql_primary` will succeed. Unqualified attempts return `400` and
**do not** change mode.
...
3. Recover either:
   ...
   - Restore healthy IQL (in tests: fix the stub), then **Resume**.
